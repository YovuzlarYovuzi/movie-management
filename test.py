...
def main() -> None:
    ...
    if response.status_code == 200:
        ...
        if "response" in data:  # Bu yerda response deyilgan kalitni tekshiramiz
            ...
            print(
                "Video api ga murojaat qilishda xatolik: {}".format(data["error"]["error_msg"])
            )  # Bu yerda xatolik haqidagi ma'lumotni konsolga chiqaramiz
    else:
        print(
            "Video api ga murojaat qilishda xatolik"
        )  # Bu yerda video api ga murojaat qilishda xatolik bo'lganligini konsolga chiqaramiz
